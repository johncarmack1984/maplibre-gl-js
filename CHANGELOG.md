...
- Tile source `bounds`, feature queries, image sources, GeoJSON data, terrain and hillshade follow a projection registered with `addProjection` ([#168](https://github.com/maplibre/maplibre-gl-js/issues/168))
- Add a `{bbox}` tile URL token that expands to the tile's bounds in the map projection's CRS units ([#168](https://github.com/maplibre/maplibre-gl-js/issues/168))
...
- Add `map.getStyleUrl()`, which returns the URL the style was loaded from, or `null` when the style was given as an object ([#7109](https://github.com/maplibre/maplibre-gl-js/issues/7109)) (by [@bradymadden97](https://github.com/bradymadden97) and [@giswqs](https://github.com/giswqs))
...
- Fix `setTiles` producing stale tile URLs when `loadTile` runs in the same frame ([#8323](https://github.com/maplibre/maplibre-gl-js/pull/8323)) (by [@johncarmack1984](https://github.com/johncarmack1984) and [@nostrorom](https://github.com/nostrorom))
...
- Log style validation warnings instead of treating them as errors, so that a filter mixing legacy and expression syntax no longer aborts the style load and blanks the map ([#7941](https://github.com/maplibre/maplibre-style-spec/pull/7941)) (by [@HarelM](https://github.com/HarelM))
...
- Fix camera jump on dragend with globe + terrain at low pitch ([#7736](https://github.com/maplibre/maplibre-gl-js/pull/7736)) (by [@kodeezabdullah](https://github.com/kodeezabdullah))
...
- Make `setTransformRequest` accept an async function in addition to a sync function. ([#7184](https://github.com/maplibre/maplibre-gl-js/issues/7184)) (by [@kikuomax](https://github.com/kikuomax))
