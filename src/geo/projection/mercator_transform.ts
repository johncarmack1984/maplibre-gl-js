...
export class MercatorTransform implements ITransform {
    ...
    private _constrainToWorldSquare(lngLat: LngLat, zoom: number): {center: LngLat; zoom: number} {
        ...
        const box = lngRange && latRange ?
            lngLatBoxToWorldBox(helper, lngRange[0], latRange[0], lngRange[1], latRange[1]) :
        ...
        const {x, y} = projectToWorldCoordinates(worldSize, lngLat, helper);

        const scale = Math.max(scaleX, scaleY);
        if (scale) {
            const newPoint = new Point(scaleX ? (maxX + minX) / 2 : x, scaleY ? (maxY + minY) / 2 : y);
            return {center: unprojectFromWorldCoordinates(worldSize, newPoint, helper), zoom: zoom + scaleZoom(scale)};
        }
        ...
        if (constrainedX !== x || constrainedY !== y) {
            return {center: unprojectFromWorldCoordinates(worldSize, new Point(constrainedX, constrainedY), helper), zoom};
        }
