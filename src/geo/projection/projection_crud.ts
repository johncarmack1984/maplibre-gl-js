 ...
 * @throws Error if the name is already registered or is one of the built-in projections, or if `tileMatrix.extentAtZoom0` or `metersPerUnit` is not positive
 * @example
 * ```ts
...
export function addProjection(def: CrsDefinition): void {
    ...
    if (def.metersPerUnit !== undefined && !(def.metersPerUnit > 0)) {
        throw new Error(`The projection "${def.name}" needs a metersPerUnit greater than zero.`);
    }
