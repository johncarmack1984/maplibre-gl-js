...
import type {WorldCoordinateHelper} from '../geo/projection/world_coordinate_helper.ts';
 ...
 */
export function reprojectGeoJSONToPseudoLngLat<T extends GeoJSON.GeoJSON>(data: T, helper: WorldCoordinateHelper): T {
    return mapGeoJSONPositions(data, (lng, lat) => {
        const {x, y} = helper.worldFromLngLat(lng, lat);
 ...
 */
export function reprojectGeoJSONFromPseudoLngLat<T extends GeoJSON.GeoJSON>(data: T, helper: WorldCoordinateHelper): T {
    return mapGeoJSONPositions(data, (pseudoLng, pseudoLat) => {
        const lngLat = helper.lngLatFromWorld(mercatorXfromLng(pseudoLng), mercatorYfromLat(pseudoLat));
