...
describe('tileIdToLngLatBounds', () => {
    test('tile 0/0/0 covers the full world', () => {
        const bounds =  tileIdToLngLatBounds(new CanonicalTileID(0, 0, 0), 0, mercatorWorldCoordinates);
    ...
    test('tile 1/1/1', () => {
        const bounds = tileIdToLngLatBounds(new CanonicalTileID(1, 1, 1), 0, mercatorWorldCoordinates);
    ...
    test('with buffer', () => {
        const bounds = tileIdToLngLatBounds(new CanonicalTileID(1, 0, 0), 0.25, mercatorWorldCoordinates);
        const boundsNoBuffer = tileIdToLngLatBounds(new CanonicalTileID(1, 0, 0), 0, mercatorWorldCoordinates);
...
describe('tileIdToLngLatBounds with a non-cylindrical world coordinate helper', () => {
    test('follows the helper, not mercator', () => {
        ...
        const mercatorBounds = tileIdToLngLatBounds(tileID, 0, mercatorWorldCoordinates);
