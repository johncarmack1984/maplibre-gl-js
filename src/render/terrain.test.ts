...
describe('Terrain in a planar projection', () => {
    function createPlanarTerrain() {
        const transform = new MercatorTransform({worldCoordinateHelper: new PlanarProjection(simpleCrs).worldCoordinateHelper});
    ...
    test('finds the tile of a location with the projection', () => {
        ...
        expect(worldX).toBe(0.75 * 2 * EXTENT);
        expect(worldY).toBe(0.25 * 2 * EXTENT);
    });

    test('does not wrap longitude: outside the world square there is no terrain', () => {
        const terrain = createPlanarTerrain();
        terrain.getElevation = () => 1;
        terrain.getMinMaxElevation = () => ({minElevation: 1, maxElevation: 42});
    ...
    test('skirt length comes from the CRS world width in meters', () => {
        const terrain = createPlanarTerrain();
        // the simple CRS world is 180 units of 1 meter
        expect(terrain.getSkirtLength(0)).toBe(180 / 5);
        expect(terrain.getSkirtLength(1)).toBe(180 / 2 / 5);
    });
});

describe('sampleAt', () => {
    function createIndex(): TerrainCoverageIndex {
        // tile 0/0/0 in world copies 0 and 1, 5 meters high everywhere
        const tileIDs = [new OverscaledTileID(0, 0, 0, 0, 0), new OverscaledTileID(0, 1, 0, 0, 0)];
        return createDEMTerrain(tileIDs, createDEM(() => 5)).getCoverageIndex();
    }

    test('wraps x into world copies when the projection wraps', () => {
        const index = createIndex();
        expect(sampleAt(index, 1, 0.5, 0.5, true)).toMatchObject({covered: true, elevation: 5});
        expect(sampleAt(index, 1, 1.5, 0.5, true)).toMatchObject({covered: true, elevation: 5});
    });

    test('is not covered outside the world square when the projection does not wrap', () => {
        const index = createIndex();
        expect(sampleAt(index, 1, 0.5, 0.5, false)).toMatchObject({covered: true, elevation: 5});
