...
export class Terrain {
    ...
    getElevationForLngLatZoom(lnglat: LngLat, zoom: number): number {
        const helper = this._worldCoordinateHelper;
        if (!isInBoundsForZoomLngLat(zoom, helper.wraps ? lnglat.wrap() : lnglat, helper)) return 0;
    ...
    getElevationForLngLat(lnglat: LngLat, transform: IReadonlyTransform): number {
        ...
        if (index) {
            ...
            const sample = sampleAt(index, this.exaggeration, x, y, helper.wraps);
    ...
    getSkirtLength(zoom: number): number {
        ...
        const helper = this._worldCoordinateHelper;
        const worldWidthInMeters = helper === mercatorWorldCoordinates ? 2 * Math.PI * earthRadius : helper.metersPerWorldUnit(this.painter.transform.center);
        return worldWidthInMeters / Math.pow(2, Math.max(zoom, 0)) / 5;
    }
    ...
    _getOverscaledTileIDFromLngLatZoom(lnglat: LngLat, zoom: number): { tileID: OverscaledTileID; worldX: number; worldY: number} {
        const helper = this._worldCoordinateHelper;
        const location = helper.wraps ? lnglat.wrap() : lnglat;
        const worldCoordinate = helper.worldFromLngLat(location.lng, location.lat);
