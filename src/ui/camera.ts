...
export class Camera extends Evented<MapEventType> {
    ...
    calculateCameraOptionsFromTo(from: LngLatLike, altitudeFrom: number, to: LngLatLike, altitudeTo: number = 0): CameraOptions {
        const helper = this.transform.worldCoordinateHelper;
        const fromLngLat = LngLat.convert(from);
        const toLngLat = LngLat.convert(to);
        const fromWorld = helper.worldFromLngLat(fromLngLat.lng, fromLngLat.lat);
        const toWorld = helper.worldFromLngLat(toLngLat.lng, toLngLat.lat);
        const dx = toWorld.x - fromWorld.x;
        const dy = toWorld.y - fromWorld.y;
        const dz = helper.worldZFromAltitude(altitudeTo, toLngLat) - helper.worldZFromAltitude(altitudeFrom, fromLngLat);
        ...
        return {
            center: helper.lngLatFromWorld(toWorld.x, toWorld.y),
