...
- There are no world copies and no antimeridian wrap: `renderWorldCopies` is always off and coordinates never wrap.
...
| `metersPerUnit` | How many meters one CRS unit is. Defaults to 1, which is right for a CRS in meters. It converts altitudes and elevations given in meters to world units and the camera distance to a zoom level, so a CRS in feet or degrees still takes meters at the API. |
| `bounds` | Optional lng/lat bounds `[west, south, east, north]` the camera is constrained to. Defaults to the tile 0/0/0 square. |
