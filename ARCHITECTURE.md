...
The transform owns a `WorldCoordinateHelper` that maps lng/lat to and from world coordinates (the unit square that tile 0/0/0 covers): mercator for the mercator, globe and vertical-perspective projections, and a mapping built from the registered CRS definition for a planar CRS registered with `addProjection`. `MercatorTransform`, the camera helper, covering tiles, sources and queries all go through that helper rather than calling the mercator functions directly, which is what lets a `PlanarProjection` render pre-projected tiles from another CRS with the mercator rendering path untouched. See `developer-guides/custom-crs.md` for the seam, the bit-identity rule that keeps mercator unchanged, and the GeoJSON pre-projection trick.
